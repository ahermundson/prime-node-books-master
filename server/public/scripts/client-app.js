...
function appendBooks(books) {
  ...
  for (var i = 0; i < books.length; i++) {
    ...
    $el.append('<button class="update btn">Update</button>');
    $el.append('<button class="delete btn">Delete</button>');
  }
}


function updatedGenreList(event) {
  event.preventDefault();
...
function getGenreList() {
  $.ajax({
    ...
    error: function() {
      console.log('Genre Database error');
    }
  });
}
...
function appendGenres(genres) {
  $('#selectedGenre').empty();
  $('#selectedGenre').append('<option value="all">ALL</option>');
