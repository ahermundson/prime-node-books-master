...
router.get('/genre', function(req,res) {
  console.log('got to genre get request');
  pg.connect(connectionString, function(err, client, done) {
    if(err) {
      console.log('connection error: ', err);
      res.sendStatus(500);
    }
    client.query('SELECT DISTINCT genre FROM books',
    function(err, result) {
      done();
      if(err) {
...
//genre select get request
router.get('/:genre', function(req, res) {
  var genre = req.params.genre;
  // console.log("is this consoling", genre);
  ...
  pg.connect(connectionString, function(err, client, done) {
    ...
    //SELECT books where the genre equals the genre selected.
